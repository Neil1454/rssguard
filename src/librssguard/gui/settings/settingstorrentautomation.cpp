...
void SettingsTorrentAutomation::loadUi() {
  ...
  auto* exclusiveIntro = new QLabel(
    tr("<b>Exclusive Batch Mode</b> starts its first monitoring cycle immediately, establishes a fresh RSS baseline, collects only genuinely new releases, sends a batch across enabled clients, then sleeps for the configured interval. While enabled it freezes all normal unattended routing, retries, retention, cleanup and deletion—even while sleeping. Manual sends remain available."),
    exclusivePage);
  ...
  m_exclusiveStrategy = new QComboBox(exclusivePage);
  for (int i = 0; i <= static_cast<int>(TorrentRoutingStrategy::Balanced); ++i)
  ...
  auto* exclusiveDependencies = new QLabel(
    tr("<b>Also used by this mode:</b> enabled clients and their capacity/active-download limits from <i>Clients and limits</i>, the assumed unknown torrent size from <i>General</i>, and duplicate protection. Cleanup, deletion, retention, normal schedules and normal retries are deliberately disabled."),
    exclusivePage);
  exclusiveDependencies->setWordWrap(true);
  exclusiveDependencies->setTextFormat(Qt::RichText);
  exclusiveDependencies->setStyleSheet(informationCardStyle(exclusiveDependencies->palette(), QStringLiteral("exclusiveDependencies")));
  exclusiveLayout->addWidget(exclusiveDependencies);
...
void SettingsTorrentAutomation::runSetupWizard() {
  ...
  auto* exclusiveRouting = new QComboBox(exclusive);
  for (int index = 0; index < m_exclusiveStrategy->count(); ++index)
    exclusiveRouting->addItem(m_exclusiveStrategy->itemText(index), m_exclusiveStrategy->itemData(index));
  exclusiveRouting->setCurrentIndex(exclusiveRouting->findData(m_exclusiveStrategy->currentData()));
