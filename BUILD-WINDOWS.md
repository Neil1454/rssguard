...
The workflow runs when the build-trigger file changes on `feature/torrent-automation` and can also be started manually. It uses the current upstream `build-windows.ps1` and `package-windows.ps1`, including recursive submodules, Qt deployment, OpenSSL, WebEngine, plugins, article extractor, ICU, and libmpv. Build 74 uses the temporary package-version tag `5.2.6-torrent-build74`. The resulting test binaries are unsigned; Windows may show a SmartScreen warning.
